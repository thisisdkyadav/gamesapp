...
const Cube = ({
  ...
  const { boardState, handleCubeClick, gameColorCode, isMove, users, colorsUser } = useLudo()
  // const { boardState } = useLudoGameManager()
  ...
  return (
    <>
      <div
        ...
        className={`${home ? "home-cube-div" : "cube"} b-${color} ${
          isStar ? `${color[0]}c-star` : ""
        } ${color}-hover`}
      >
        {noOfPawns > 0 &&
