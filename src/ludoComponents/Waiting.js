...
const Waiting = ({ gameID, users, username }) => {
  const [message, setMessage] = useState("")
  const inputEmail = useRef()
  const startGame = () => {
    if (
      Object.keys(users).filter((user) => users[user] === "joined").length > 1
    ) {
      update(ref(db), {
  ...
  return (
    <div className="waiting">
      ...
      <div className="grid">
        ...
        <div className="grid-c2" onClick={inviteUser}>
          +
        </div>
        {users &&
          Object.keys(users).map((user, index) => {
            return (
              <>
                <div
                  className={username === gameID ? "grid-c1" : "grid-c12"}
                  key={index}
                >
                  ({users[user]}){user.replace(/\_/g, ".")}
                </div>
                {username === gameID || username === user ? (
                  <div
                    data-user={user}
                    data-status={users[user]}
                    className="grid-c2"
                    onClick={removeUser}
                  >
                    -
        ...
        {username === gameID ? (
          <>
            ...
            <div>
              <div className="grid-c2" onClick={exitGame}>
                x
              </div>
            </div>
          </>
