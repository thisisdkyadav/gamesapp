...
const Invites = () => {
  const { username, invites } = useLudo()
        ...
        {invites &&
          Object.keys(invites).map((game, index) => {
            if (invites[game]) {
              ...
              return (
                <>
                  <div className="grid-c1" key={index}>
                    {game.replace(/\_/g, ".")}{" "}
                  </div>
                  <div>
                    <div className="grid-c2" onClick={() => acceptInvite(game)}>
                      +
                    </div>{" "}
                  </div>
                  <div>
                    <div className="grid-c2" onClick={() => rejectInvite(game)}>
                      -
                    </div>
                  </div>
                </>
