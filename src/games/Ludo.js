...
import { LudoProvider } from "../context/ludo"
import { LudoGameManagerProvider } from "../context/ludoGameManager"
    ...
    <>
      ...
      <div className="main">
        <LudoProvider>
          ...
          {/* {status !== "active" && Object.keys(invites).length > 0 ? (
              <Invites />
            ) : (
              ""
            )}
            ...
            {status === "waiting" ? (
              <Waiting gameID={gameID} users={users} username={username} />
            ) : (
              ""
            )}
        ...
        >
          {status !== "active" && Object.keys(invites).length > 0 ? (
            <Invites invites={invites} username={username} />
          ) : (
            ""
          )}
